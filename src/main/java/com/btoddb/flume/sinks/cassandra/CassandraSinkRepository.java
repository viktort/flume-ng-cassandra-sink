...
public class CassandraSinkRepository {
	...
	public void saveToCassandra(List<Event> eventList) {
		com.btoddb.flume.sinks.cassandra.CassandraJob job = new com.btoddb.flume.sinks.cassandra.CassandraJob(keyspace, workExecutor);
		for (Event event : eventList) {
			com.btoddb.flume.sinks.cassandra.FlumeLogEvent flumeLog = new com.btoddb.flume.sinks.cassandra.FlumeLogEvent(event);

			long tsInMillis = flumeLog.getTimestamp();
			String src = flumeLog.getSource();
			String host = flumeLog.getHost();
			String key = flumeLog.getKey();
			String headerEvent = flumeLog.getHeaderEvent();
			String headerEventType = flumeLog.getHeaderEventType();
			...
			m.addInsertion(recordsKey, recordsColFamName, HFactory.createColumn("event", headerEvent));
			m.addInsertion(recordsKey, recordsColFamName, HFactory.createColumn("event_type", headerEventType));
			job.submitWorkUnit();
		}
